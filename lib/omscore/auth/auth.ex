defmodule Omscore.Auth do
  ...
  # "Logs" login attempts to prevent brute-forcing
  def login_bruteforce_protection(remote_ip, username, password) do
    now = DateTime.utc_now() |> DateTime.to_unix()

    # Hash password so we don't store passwords in plaintext
    password = :crypto.hash(:sha, password)

    # This removes all entries which are older than 5 minutes. Most unreadable syntax ever
    # Was generated by using :ets.fun2ms(fn({_, _, _, t}) when t < 3 -> true end)
    :ets.select_delete(:login_attempts, [{{:_, :_, :_, :"$1"}, [{:<, :"$1", now - Application.get_env(:omscore, :login_attempt_decay)}], [true]}])

    # Add current attempt
    :ets.insert(:login_attempts, {remote_ip, username, password, now})

    # Check if that ip, username or password exceeded the maximum number of requests
    # Even more unreadable syntax
    # Was generated using :ets.fun2ms(fn({x, y, z, t}) when x == 1 or y == 2 or z == 3 -> t end)
    attempts = :ets.select(:login_attempts, [{{:"$1", :"$2", :"$3", :"$4"}, [{:orelse, {:orelse, {:==, :"$1", remote_ip}, {:==, :"$2", username}}, {:==, :"$3", password}}], [:"$4"]}])
    max_tries = Application.get_env(:omscore, :max_login_attempts)

    if Enum.count(attempts) > max_tries do
      {:error, :too_many_requests, "Too many login attempts, try again a bit later"}
    else
      {:ok}
    end
  end
